...
class brains:
    ...
    def create_model(self):
        ...
        #output layer
        model.add(Dense(4,name="output")) # Q values expected for each action
    ...
    def train(self, NUM_EPISODES, EP_DURATION, hider_start_state, seeker_start_state):
        ...
        for ep in range(NUM_EPISODES):
            ...
            while running and elapsed <= GAME_DURATION:
                ...
                ######## take step
                next_state, reward, done = main.step(hider_action, seeker_action)
                ...
                if steps % self.UPDATE_EVERY_STEP == 0 and len(self.replay_buffer) > self.batch_size:
                    ...
                    # S,a,r,S'
                    for i in indices:
                        current_state_sample = self.replay_buffer[i][0]
                        hider_action_sample = self.replay_buffer[i][1]
                        reward_sample = self.replay_buffer[i][2]
                        next_state_sample = self.replay_buffer[i][3]
